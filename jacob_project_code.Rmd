...
```
```{r}
planets_temps %>% filter(PlanetIdentifier == "Earth")
```

```{r}
#Lib that works with shiny. Required to print data table by the 
...
server <- function(input, output, session) {
  ...
  output$plot <- renderPlot({
    data1 <- get_planet_data()
    ggplot(data = data1, aes_string(x = "PlanetIdentifier", y = "PlanetaryMassJpt", group = "PlanetIdentifier")) +
